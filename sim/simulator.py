...
from .planner import PlanningState, apply_action, generate_actions, score_state
...
def _resolve_post_action_effects(
    ...
    """
    Real effects are resolved here, not in planner:
    - spell-result draws
    - ETB draws
    - LTB draws caused by sacrifices
    - token creation on resolve
    """
    changed_state = state
...
def _search_best_line(
    ...
    for action in actions:
        ...
        resolved_state, _ = _resolve_post_action_effects(next_state, branch_library, event)
        line, future_score = _search_best_line(resolved_state, branch_library, depth - 1)
...
def _begin_turn(state: PlanningState, library: list, turn: int, on_the_play: bool):
    """
    Correct turn order:
    1) untap / reset turn flags
    2) clear entered_this_turn markers from last turn
    3) draw for turn if applicable

    This ensures:
    - battlefield_start reflects TU (untapped) lands
    - hand_start includes the normal draw-step card
    - evaluate_turn receives proper start-of-turn snapshots
    """
...
def simulate_game(
    ...
    *,
    ...
    for turn in range(1, num_turns + 1):
        # IMPORTANT: start-of-turn state must already be untapped before snapshots.
        state, draw_step = _begin_turn(state, library, turn, on_the_play)

        # Snapshots for metrics/logging are now the true start-of-turn state.
